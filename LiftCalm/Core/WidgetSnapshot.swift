...
//  LiftCalm
//
//  Shared contract between the app and the Home Screen widget. KEEP IN SYNC with
//  the byte-identical copy at LiftCalmWidget/WidgetSnapshot.swift — the two
//  targets each compile their own copy (no shared framework, by design).
//
//  Primitives only (no SwiftData/SwiftUI types cross the App Group boundary).
//  Readiness colors are baked in as hex so the extension needs zero app code.
//
